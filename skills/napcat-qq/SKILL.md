...
- 兼容会话键输入：`agent:<agentId>:napcat:direct:<QQ号>`（发送前应改写为 `direct:<QQ号>`）
...
4. 用户给的是 `agent:*` 完整会话键时，提取目标 ID 并改写为 `direct:` / `group:` 后再发送。
5. 发送媒体时使用 `mediaUrl`，可同时带 `text`。
6. 未提供目标 ID 时先询问，不做猜测发送。
7. 回复风格默认口语化、简洁，避免冗长输出。
